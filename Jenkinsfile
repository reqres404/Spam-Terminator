		...
		stage('Run Containers') {
			steps {
				script {
					...
					sh "docker run -d -p ${clientPort}:3000 adittyapatil1818/spam-terminator-jenkins:client"
					sh "docker run -d -p ${serverPort}:4000 adittyapatil1818/spam-terminator-jenkins:server"
				}
			}
		}
	}
